(function() {
  if (!Array.prototype.indexOf) Array.prototype.indexOf = function(r,t){var n;if(null==this)throw new TypeError('"this" is null or not defined');var e=Object(this),i=e.length>>>0;if(0===i)return-1;var a=+t||0;if(Math.abs(a)===1/0&&(a=0),a>=i)return-1;for(n=Math.max(a>=0?a:i-Math.abs(a),0);i>n;){if(n in e&&e[n]===r)return n;n++}return-1};

  var _json = (typeof JSON != 'undefined' ? JSON : {});
  ...
  var noop = function() {},
    _isIE = function() {
      try {
        ...
        return ua.indexOf('msie') != -1 ? parseInt(ua.split('msie')[1]) <= 9 : !1;
      }
      catch (ex) { return !1 }
    },
    _log_stringify = _isIE(),
    _log_support = (typeof console != 'undefined' && typeof console.log != 'undefined'),
    log = function(a) {
      if (_log_support) {
        try { console.log(a) }
        catch (ex) {}
    ...
    logError = function(a) {
      if (_log_support) {
        try {
          var args = _log_stringify ? _json._stringify([].slice.call(arguments)) : arguments;
          isFunction(console.error) ? console.error(args) : console.log(args);
        }
        catch (ex) {}
      }
    },
    logDebug = function(a) {
      if (_log_support) {
        try {
          var args = _log_stringify ? _json._stringify([].slice.call(arguments)) : arguments;
          isFunction(console.debug) ? console.debug(args) : console.log(args);
        }
        catch (ex) {}
      }
    },
  ...
  var transportHost = function() {
    ...
    beaconSendError = function(parameters, errorCode, details, errorMsg) {
      ...
      if (!optedOut) {
        var payload = ["et=error", "_err=" + errorCode];
        if (parameters) {
          payload.push("_lib=" + parameters.get(_client_library));
          payload.push("_libv=" + parameters.get(_client_version));
          payload.push("sig=" + parameters.get(_signer_id));
          payload.push("sid=" + parameters.get(_site_id));
        }
        if (details) payload.push("_dtl=" + details);
        if (errorMsg) payload.push("_msg=" + tryEncodeURIComponent(errorMsg.substring(0, 100)));
  ...
  function sendWithXHR(parameters, transportAddress, payload, eventType, context, callback, retries) {
    ...
    if ("withCredentials" in xhr) xhr.withCredentials = !0;
    retries = retries || 0;
    var err, started, dt, du, ab, ses, uuid,
      response = null,
    ...
    xhr.timeout = parameters.getNumber(_timeout) || 20000;
    xhr.setRequestHeader("Content-Type", "text/plain");
    xhr.onreadystatechange = function() {
    ...
    xhr.onload = function() {
      ...
      try {
        ...
        ses = xhr.getResponseHeader('X-Activity-Session');
        ...
        if (ses) parameters.set(_session, ses);
  ...
  function sendAction(parameters) {
    ...
    if (getParameterValue(parameters, _synchronous_sending) && transportType === "xhr" && !isRetrieve) {
      transportAddress = transportAddress + "/sync";
    }
  ...
  var generateUUID = function(parameters, uuid) {
    ...
    setSystemInfo = function(parameters) {
      ...
      parameters.set(_jurisdiction, getBrowserLocale());
    },
    ...
    detectOperatingSystem = function() {
      var os = "Unknown";
      if (!window.navigator) return os;
      var userAgent = window.navigator.userAgent;
      if (/(Windows|WinNT4.0|Win16|Win 9x 4.90)/.test(userAgent)) os = "Windows";
      else if (/(Mac OS|MacPPC|MacIntel|Mac_PowerPC|Macintosh|Mac OS X)/.test(userAgent)) os = "MacOS";
      else if (/Android/.test(userAgent)) os = "Android";
      else if (/CrOS/.test(userAgent)) os = "Chrome OS";
      else if (/(iPhone|iPad|iPod)/.test(userAgent)) os = "iOS";
      else if (/(Linux|X11(?!.*CrOS))/.test(userAgent)) os = "Linux";
      else if (/UNIX/.test(userAgent)) os = "UNIX";
      else if (/GoogleBot/.test(userAgent)) os = "GoogleBot";
      return os;
    },
    detectEnvironment = function() {
      var env = "desktop";
      if (!window.navigator) env = "other";
      else if ((/Mobile|Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i).test(window.navigator.userAgent)) env = "mobile";
      return env;
    };
  ...
  var uuidSet = !1,
    ...
    validateUUIDCookie = function(parameters) {
      if (!uuidSet && getParameterValue(parameters, _storage) == 'cookie') {
        setUUIDCookie(parameters);
        if (!uuidSet) log('Warning: Unable to set UUID cookie');
      }
    },
  ...
  var _client_version = defineProperty("client_version", "_libv"),
  ...
  var _site_id = defineParameter("site_id", "sid"),
    ...
    _timeout = defineParameter("timeout", void 0, 20000),
  ...
  var _context = defineParameter("_context"),
    ...
    _synchronous_sending = defineParameter("synchronous_sending");
  ...
  var _key = defineParameter("key", "gkey"),
    ...
    _snapshot_location = defineParameter("snapshot_location", "slo");
  ...
  var Site = function(options) {
    ...
    setParameter(_field_data, options[_field_data]);
    ...
    setParameter(_synchronous_sending, options[_synchronous_sending]);
    setParameter(_client_version, '1.2.0'); // Pulled from package.json version
    setParameter(_client_library, 'ps.js'); // PactSafe Snippet
    ...
    _listenTo('set:' + _signer_id, _bind(self.render, self));
  };
  ...
  var ClickwrapGroup = function(options) {
    ...
    setParameter(_policy, options[_policy]);
    setParameter(_target_jurisdictions, options[_target_jurisdictions]);
    setParameter(_execution_method, options[_execution_method]);
    ...
    setParameter(_snapshot_location, options[_snapshot_location]);
    ...
    _PactSafe('onReady', function() { self.listenForElement(self.get(_container_selector), _bind(self.render, self)) });

    if (self.get(_signer_id_selector)) _PactSafe('onReady', function() { self.listenForElement(self.get(_signer_id_selector), _bind(self.listenForSignerId, self)) });
    else _listenTo('set:' + _signer_id_selector, function() { self.listenForElement(self.get(_signer_id_selector), _bind(self.listenForSignerId, self)) });
    if (!self.get(_form_selector)) _listenTo('set:' + _form_selector, function() { self.listenForElement(self.get(_form_selector), _bind(self.injectHiddenInput, self)) });
  };
  ...
  ClickwrapGroup.prototype.getOverrides = function(overrides) {
    ...
    overrides[_execution_method] = overrides[_execution_method] || self.get(_execution_method);
  ...
  ClickwrapGroup.prototype.getChecked = function(skipAgreed) {
    var self = this,
      ...
      versions = self.get(_versions),
      revisions = self.get(_revisions),
      sig = self.site.get(_signer_id),
      cid = [],
      vid = [],
      rev = [];
    for (var i = 0; i < contracts.length; i++) {
      if (self.checked.data.get(contracts[i]) && (!skipAgreed || self.sent[contracts[i]] !== sig)) {
        cid.push(contracts[i]);
        vid.push(versions[i]);
        if (revisions) rev.push(revisions[i]);
      }
  ...
  ClickwrapGroup.prototype.listenForSignerId = function() {
    ...
    if (sigEl && !this.signerIdListening) {
      ...
      listenTo(sigEl, 'change', _bind(this.handleInput, this));
      listenTo(sigEl, 'autofill', _bind(this.handleInput, this));
    }
  };
  ...
  ClickwrapGroup.prototype.updateHiddenInput = function(val) {
    ...
    if (formEl && this.inputInjected) {
      ...
      if (this.get(_block_form_submission)) {
        ...
        if (submitBtn && submitBtn.setCustomValidity && isFunction(submitBtn.setCustomValidity)) {
          submitBtn.setCustomValidity(val ? '' : this.get(_alert_message));
        }
      }
    }
    if (changed) {
  ...
  ClickwrapGroup.prototype.render = function(force) {
    ...
    if (self.get(_policy)) self.buildDynamicSignerLinks();
  ...
  ClickwrapGroup.prototype.buildDynamicSignerLinks = function() {
    ...
    var el = self.get(_element);
    var links = getByClassName(el, 'ps-contract-link');
    var url = getElement('ps-group-' + self.get(_key)).getAttribute('data-legal');

    for (var i=0; i<links.length; i++) {
      var params = {};
      if (self.get(_group)) params['g'] = self.get(_group);
      if (self.site.get(_jurisdiction)) params['jur'] = self.site.get(_jurisdiction);
      if (self.site.get(_signer_id)) params['sig'] = encodeURIComponent(self.site.get(_signer_id));
      if (self.site.get(_test_mode)) params['tm'] = encodeURIComponent(self.site.get(_test_mode));
      ...
      if (links[i].getAttribute('data-ckey')) output += '#' + links[i].getAttribute('data-ckey');
      links[i].setAttribute('href', output);
    }
  };
  ...
  ClickwrapGroup.prototype.injectHTML = function(el) {
    if (el) {
      ...
      for (var i = 0; i < targets.length; i++) {
        if (targets[i].tagName == 'BUTTON') listenTo(targets[i], 'click', _bind(this.handleClick, this), !1);
        else listenTo(targets[i], 'change', _bind(this.handleClick, this), !1);
        if (panes && panes.length > 0) listenTo(targets[i], 'change', _bind(this.togglePane, this), !1);
      }
      for (var i = 0; i < inputs.length; i++) {
        listenTo(inputs[i], 'change', _bind(this.handleAttributeInput, this));
        listenTo(inputs[i], 'autofill', _bind(this.handleAttributeInput, this));
      }
      for (var b = 0; b < exp.length; b++) listenTo(exp[b], 'click', _bind(this.togglePane, this), !1);
      for (var b = 0; b < col.length; b++) listenTo(col[b], 'click', _bind(this.togglePane, this), !1);
      for (var i = 0; i < contracts.length; i++) {
        this.checked.data.set(contracts[i], !1);
      }
      this.updateHiddenInput(!this.block());
    }
  };
  ...
  ClickwrapGroup.prototype.setupScrollListeners = function() {
    var self = this;
    var el = self.get(_element);
    var scrolls = getByClassName(el, 'ps-contract-force-scroll');
    var handler = _bind(self.handleScroll, self);
  ...
  ClickwrapGroup.prototype.handleScroll = function(evt) {
    if (evt && evt.target) {
      ...
      var contracts = [].slice.call(getByClassName(target, 'ps-contract-scroll-bottom'));
      var tl = getByClassName(el, 'ps-contract-scroll-bottom').length;
      if (hasClass(target, 'ps-contract-scroll-bottom')) contracts.push(target);
      var cl = contracts.length;
      for (var i = 0; i < contracts.length; i++) {
        if (Math.ceil(1 + target.scrollTop + target.clientHeight) >= contracts[i].scrollHeight) { // Add 1 to scrollTop, display scaling may result in decimal and off by 1 values.
          cl -= 1;
          tl -= 1;
  ...
  ClickwrapGroup.prototype.unblockScroll = function(el) {
    ...
    if (el) {
      ...
      el = this.get(_element);
      scrollmsg = getByClassName(el, 'ps-scroll-message');
      checkbox = getByClassName(el, 'ps-checkbox-clickable');
      for (i = 0; i < scrollmsg.length; i++) scrollmsg[i].style.display = 'none';
      for (i = 0; i < checkbox.length; i++) checkbox[i].style.display = 'block';
  ...
  ClickwrapGroup.prototype.retrieveHTML = function(renderData) {
    ...
    xhr.onreadystatechange = function() {
      if (4 == xhr.readyState) {
        try {
          var rdid = xhr.getResponseHeader('X-Render-ID');
          if (rdid) self.set(_render_id, rdid);
        }
        catch (err) {}
        self.set(_contract_html, xhr.responseText);
        self.resetData();
        self.render(true);
        xhr = null;
      }
  ...
  ClickwrapGroup.prototype.display = function() {
    ...
    else if (!this.get(_policy)) this.displayRequired();
    else {
      var targets = this.get(_target_jurisdictions);
      if (isArray(targets) && targets.length) {
        if (targets.indexOf(this.site.get(_jurisdiction)) > -1) this.displayRequired();
  ...
  ClickwrapGroup.prototype.displayRequired = function() {
    ...
    if (el) {
      ...
      if (this.get(_policy)) this.displayOptOut(cel);
      this.sendDisplayed();
    }
  };
  ...
  ClickwrapGroup.prototype.displayOptOut = function(cel) {
    for (var i = 0; i < cel.length; i++) {
      cid = (getAttribute(cel[i], 'data-' + this.get(_data_attr)) || '').split(',');
      if (cid.length) {
        for (var c = 0; c < cid.length; c++) {
          if (this.agreed.data.get(cid[c]) === !0) {
            var labels = cel[i].parentElement.getElementsByClassName('opt-out-container');
  ...
  ClickwrapGroup.prototype.handleClick = function(evt) {
    if (evt && evt.target) {
      var self = this,
        ...
        rev = [],
        data = {},
        checked = evt.target.tagName == 'BUTTON' ? true : evt.target.checked,
        pairs = evt.target.value.split(',');
      for (var i = 0, v = null; i < pairs.length; i++) {
        v = pairs[i].split(':');
        if (checked && self.sent[v[0]] !== sig) {
          cid.push(v[0]);
          vid.push(v[1]);
          if (v.length > 2) rev.push(v[2]);
        }
        self.checked.data.set(v[0], checked);
      }
      emit((checked ? 'checked' : 'unchecked'), self, evt.target);
      if (cid.length) {
        data[_contracts] = cid;
        data[_versions] = vid;
  ...
  ClickwrapGroup.prototype.handleAttributeInput = function(evt) {
    if (evt && evt.target) {
      var input = evt.target,
        sigEl = getElement(this.get(_signer_id_selector));
      if (hasClass(input, 'ps-attr')) {
        var attr = getAttribute(input, 'data-field') || getAttribute(input, 'name');
        if (!isBlank(attr) && attr != 'signer_id') {
  ...
  var BrowsewrapGroup = function(options) {
    ...
    _PactSafe('onReady', function() { self.listenForElement(self.get(_target_selector), _bind(self.render, self)) });
  ...
  BrowsewrapGroup.prototype.extend = function(name, value) {
    this.parameters.extend(name, value);
  };

  BrowsewrapGroup.prototype.getParams = function(overrides) {
    var self = this;
    overrides = self.getOverrides(overrides);
    return self.site.getParams(overrides);
  };

  BrowsewrapGroup.prototype.getPayload = function(overrides) {
    var self = this;
    overrides = self.getOverrides(overrides);
    return self.site.getPayload(overrides);
  };

  BrowsewrapGroup.prototype.getOverrides = function(overrides) {
    var self = this;
    if (!overrides || "object" != typeof overrides) overrides = {};
    overrides[_contracts] = overrides[_contracts] || self.get(_contracts);
    overrides[_versions] = overrides[_versions] || self.get(_versions);
  ...
  BrowsewrapGroup.prototype.wrapElement = function() {
    ...
    if (this.get(_open_legal_center)) {
      ...
      listenTo(el, 'click', _bind(this.openLegalCenter, this), !1);
      listenTo(badgeEl, 'click', _bind(this.openLegalCenter, this), !1);
    }
  ...
  _PactSafe.envVault = 'local-vault.pactsafe.io';
